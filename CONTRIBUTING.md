...
**GPU volume/strip field assists:** when an effect uses `SpatialVolumeFieldAssist` / `SpatialStripFieldAssist`, the atlas is the source of truth. Do not keep parallel CPU formula fallbacks. Sample coords reaching `CalculateColorGrid` are already axis-scaled and rotated by the render path (`ApplyEffectRotation`) unless `SkipsSpatialSampleWarp()` is true. Origin-local atlas UV (`SampleGpuVolumeOriginLocal01`) uses origin→farthest-face half-extents so scale=1 covers the grid from any Spatial Anchor — do not shrink that domain by Size or stratum tightness (pass those as `u_params`). Single-atlas height bands bake mid-band stratum scalars at `PrepareGpuFields` — full per-LED multi-atlas stratum is out of scope until designed deliberately.
