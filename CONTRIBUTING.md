...
**Exception — viewport only:** the Qt **OpenGL 4.1 Core** room (`LEDViewport3D` / `QOpenGLWidget` + `QOpenGLFunctions_4_1_Core`) is the only viewport path (no GPU shader dual path). That is rendering infrastructure, not data-format backward compatibility. Requires **Qt 6.8 or newer**, built with the same Qt as the OpenRGB host (6.8.3 official Windows/AppImage; 6.10.x on CachyOS/Arch and the KDE Flatpak runtime).
