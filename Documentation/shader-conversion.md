...
GPU for room fields — including **audio visual fields** (Audio Level, Spectrum Bars, Strip Viz, Pulse) and **Screen Mirror** via `SpatialVolumeFieldAssist` + optional `u_media`. Screen Mirror pairs `SampleGpuRoomVolume01` with plane `map_uv` (display-plane room UV) and Spatial Anchor `falloff_uv`. CPU keeps **analysis** (FFT / bands / onset in `AudioInputManager`), screen **capture** (not the room map), Minecraft, and cheap color finish after atlas sample (Screen Mirror: capture + LED temporal smoothing + 2D color grade before `u_media` upload).
