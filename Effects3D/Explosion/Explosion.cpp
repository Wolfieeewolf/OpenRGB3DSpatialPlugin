...
void Explosion::SetupCustomUI(QWidget* parent)
{
    ...
    type_combo = type_row->combo();
    type_combo->setToolTip(QStringLiteral(
        "Shockwave recipe and debris style. Particles slider mainly affects Standard and Bomb."));
    type_combo->addItem(QStringLiteral("Standard"));
    type_combo->setItemData(0, QStringLiteral("Expanding wave plus optional debris."), Qt::ToolTipRole);
    type_combo->addItem(QStringLiteral("Nuke"));
    type_combo->setItemData(1, QStringLiteral("Tall mushroom-style column and cap."), Qt::ToolTipRole);
    type_combo->addItem(QStringLiteral("Land Mine"));
    type_combo->setItemData(2, QStringLiteral("Low ground-hugging burst."), Qt::ToolTipRole);
    type_combo->addItem(QStringLiteral("Bomb"));
    type_combo->setItemData(3, QStringLiteral("Symmetric fireball with debris."), Qt::ToolTipRole);
    type_combo->addItem(QStringLiteral("Wall Bounce"));
    type_combo->setItemData(4, QStringLiteral("Wave reflects when it hits room bounds."), Qt::ToolTipRole);
    ...
    EffectSliderRow* intensity_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Intensity:"),
        10,
        200,
        explosion_intensity,
        QStringLiteral("Explosion energy (radius and wave thickness)"));
    intensity_row->setObjectName(QStringLiteral("intensityRow"));
    intensity_slider = intensity_row->slider();
    intensity_row->bindValueChanged(
        this,
        [this](int v) {
            explosion_intensity = v;
            OnExplosionParameterChanged();
        },
        [](int v) { return QString::number(v); },
        nullptr);

    EffectSliderRow* particles_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Particles:"),
        0,
        100,
        particle_amount,
        QStringLiteral("Debris/spark density for Standard and Bomb (0 = wave only)"));
    particles_row->setObjectName(QStringLiteral("particlesRow"));
    particle_slider = particles_row->slider();
    particles_row->bindValueChanged(
        this, [this](int v) { particle_amount = v; }, pct_format, on_changed);

    EffectLabeledSpinRow* burst_count_row = EffectUiRows::AppendSpinRow(
        layout, QStringLiteral("Burst count:"), 0, 10, burst_count,
        QStringLiteral("Number of explosions per cycle (0 = never stop)"));
    burst_count_spin = burst_count_row->spinBox();
    burst_count_spin->setObjectName(QStringLiteral("burstCountSpin"));
    burst_count_spin->setSpecialValueText(QStringLiteral("Infinite"));
    burst_count_row->bindValueChanged(this, [this](int v) {
        burst_count = v;
        OnExplosionParameterChanged();
    });

    loop_check = new QCheckBox(QStringLiteral("Loop (repeat; ties to color change)"), w);
    loop_check->setObjectName(QStringLiteral("loopCheck"));
    loop_check->setChecked(loop);
    layout->addWidget(loop_check);
    loop_check->setToolTip(QStringLiteral(
        "When on, after burst count repeats and hue advances. When off, effect ends after that many explosions."));
    connect(loop_check, &QCheckBox::toggled, this, [this](bool checked) {
        loop = checked;
