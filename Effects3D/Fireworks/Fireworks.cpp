...
{
    QWidget* w = EffectUiRows::NewEffectPanel("FireworksEffectSettings");
    QVBoxLayout* layout = EffectUiRows::PanelLayout(w);
    const auto on_changed = [this]() { emit ParametersChanged(); };
    const auto pct_format = [](int v) { return QString::number(v) + QStringLiteral("%"); };

    EffectLabeledComboRow* type_row = EffectUiRows::AppendComboRow(layout, QStringLiteral("Type:"));
    type_row->setObjectName(QStringLiteral("typeRow"));
    QComboBox* type_combo = type_row->combo();
    for(int t = 0; t < TYPE_COUNT; t++)
    {
        type_combo->addItem(TypeName(t));
    }
    type_combo->setCurrentIndex(std::max(0, std::min(firework_type, TYPE_COUNT - 1)));
    type_combo->setToolTip(QStringLiteral("Launch and burst personality. Random cycles mixed styles each cycle."));
    type_combo->setItemData(0, QStringLiteral("One classic mortar burst."), Qt::ToolTipRole);
    type_combo->setItemData(1, QStringLiteral("Large aerial burst."), Qt::ToolTipRole);
    type_combo->setItemData(2, QStringLiteral("Repeated upward shots from low height."), Qt::ToolTipRole);
    type_combo->setItemData(3, QStringLiteral("Spinning fountain-style emission."), Qt::ToolTipRole);
    type_combo->setItemData(4, QStringLiteral("Upward spray that falls with gravity."), Qt::ToolTipRole);
    type_combo->setItemData(5, QStringLiteral("Pick a different style each cycle."), Qt::ToolTipRole);
    ...
    EffectLabeledComboRow* burst_style_row = EffectUiRows::AppendComboRow(layout, QStringLiteral("Burst style:"));
    burst_style_row->setObjectName(QStringLiteral("burstStyleRow"));
    QComboBox* burst_combo = burst_style_row->combo();
    for(int s = 0; s < BURST_COUNT; s++)
    {
        burst_combo->addItem(BurstStyleName(s));
    }
    burst_combo->setCurrentIndex(std::max(0, std::min(burst_style, BURST_COUNT - 1)));
    burst_combo->setToolTip(QStringLiteral("Shape profile for aerial shell particles."));
    burst_combo->setItemData(0, QStringLiteral("Blend styles automatically per launch."), Qt::ToolTipRole);
    burst_combo->setItemData(1, QStringLiteral("Classic spherical shell."), Qt::ToolTipRole);
    burst_combo->setItemData(2, QStringLiteral("Round shell with dense outer ring."), Qt::ToolTipRole);
    burst_combo->setItemData(3, QStringLiteral("Slow, drooping trails."), Qt::ToolTipRole);
    burst_combo->setItemData(4, QStringLiteral("Upward-heavy palm fronds."), Qt::ToolTipRole);
    burst_combo->setItemData(5, QStringLiteral("Small branch splits from each star."), Qt::ToolTipRole);
    ...
    auto bind_slider = [&](const char* name, const QString& caption, int min, int max, int value,
    ...
    bind_slider("simultaneousRow", QStringLiteral("Simultaneous:"), 1, 10, num_simultaneous,
                QStringLiteral("How many bursts can be active at once (more = busier, heavier)."),
                [this](int v) { num_simultaneous = v; }, [](int v) { return QString::number(v); });
    bind_slider("showDensityRow", QStringLiteral("Show density:"), 50, 200, (int)(show_density * 100.0f),
                QStringLiteral("How packed the overall show is (active launches and particle volume)."),
                [this](int v) { show_density = v / 100.0f; }, pct_format);
    bind_slider("showVarietyRow", QStringLiteral("Show variety:"), 0, 100, (int)(show_variety * 100.0f),
                QStringLiteral("How much shell size/height/position variation is added across launches."),
                [this](int v) { show_variety = v / 100.0f; }, pct_format);
    bind_slider("particleCountRow", QStringLiteral("Particle count:"), 15, 100, num_debris,
                QStringLiteral("Debris particle count for styles that spawn sparks (not all types use it equally)."),
                [this](int v) { num_debris = v; }, [](int v) { return QString::number(v); });
    bind_slider("particleSizeRow", QStringLiteral("Particle size:"), 2, 100, (int)(particle_size * 100.0f),
                QStringLiteral("Visual size of debris streaks and sparks."),
                [this](int v) { particle_size = v / 100.0f; }, pct_format);
    bind_slider("gravityRow", QStringLiteral("Gravity:"), 0, 200, (int)(gravity_strength * 100.0f),
                QStringLiteral("How strongly particles fall after burst (0 = floaty)."),
                [this](int v) { gravity_strength = v / 100.0f; }, pct_format);
    bind_slider("decaySpeedRow", QStringLiteral("Decay speed:"), 50, 600, (int)(decay_speed * 100.0f),
                QStringLiteral("How quickly sparks and trails fade out."),
                [this](int v) { decay_speed = v / 100.0f; },
                [this](int v) {
                    (void)v;
                    return QString::number(decay_speed, 'f', 1);
                });
    bind_slider("launchTrailRow", QStringLiteral("Launch trail:"), 0, 200,
                (int)std::lround(launch_trail_amount * 100.0f),
                QStringLiteral("Comet tail strength during launch."),
                [this](int v) { launch_trail_amount = v / 100.0f; }, pct_format);
    bind_slider("burstFlashRow", QStringLiteral("Burst flash:"), 0, 200,
                (int)std::lround(burst_flash_amount * 100.0f),
                QStringLiteral("Brightness and duration of the burst ignition flash."),
                [this](int v) { burst_flash_amount = v / 100.0f; }, pct_format);
    bind_slider("emberAmountRow", QStringLiteral("Ember amount:"), 0, 200,
                (int)std::lround(ember_amount * 100.0f),
                QStringLiteral("How strong the post-burst ember layer is."),
                [this](int v) { ember_amount = v / 100.0f; }, pct_format);
    bind_slider("emberHangRow", QStringLiteral("Ember hang time:"), 40, 220,
                (int)std::lround(ember_hang_time * 100.0f),
                QStringLiteral("How long embers linger before fading."),
                [this](int v) { ember_hang_time = v / 100.0f; }, pct_format);
...
void Fireworks::LoadSettings(const nlohmann::json& settings)
{
    ...
    {
        if(QWidget* fx = EffectUiSync::effectPanel(panel, "FireworksEffectSettings"))
        {
            EffectUiSync::setComboIndex(fx, "typeRow", firework_type);
            EffectUiSync::setComboIndex(fx, "burstStyleRow", burst_style);
            const auto pct = [](int v) { return QString::number(v) + QStringLiteral("%"); };
            EffectUiSync::setSliderValue(fx, "simultaneousRow", num_simultaneous, [](int v) { return QString::number(v); });
            EffectUiSync::setSliderValue(fx, "showDensityRow", (int)(show_density * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "showVarietyRow", (int)(show_variety * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "particleCountRow", num_debris, [](int v) { return QString::number(v); });
            EffectUiSync::setSliderValue(fx, "particleSizeRow", (int)(particle_size * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "gravityRow", (int)(gravity_strength * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "decaySpeedRow", (int)(decay_speed * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "launchTrailRow", (int)std::lround(launch_trail_amount * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "burstFlashRow", (int)std::lround(burst_flash_amount * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "emberAmountRow", (int)std::lround(ember_amount * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "emberHangRow", (int)std::lround(ember_hang_time * 100.0f), pct);
        }
    }
}
