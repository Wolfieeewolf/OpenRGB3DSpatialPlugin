...
void SettingsToJson(const Settings& s, nlohmann::json& j)
{
    ...
    put_bool(j, "room_vr_flip_right", s.room_vr_flip_right);
    put_bool(j, "room_vr_flip_forward", s.room_vr_flip_forward);
    put_i(j, "room_vr_cubemap_face_size", s.room_vr_cubemap_face_size);
    put_i(j, "room_vr_texture_uv_dim", s.room_vr_texture_uv_dim);
...
void SettingsFromJson(const nlohmann::json& j, Settings& s)
{
    ...
    s.room_vr_flip_right = get_bool(j, "room_vr_flip_right", s.room_vr_flip_right);
    s.room_vr_flip_forward = get_bool(j, "room_vr_flip_forward", s.room_vr_flip_forward);
    // Obsolete keys (world tint / lightning / sharp_sampling / old cubemap toggles) are ignored.
    s.room_vr_cubemap_face_size = get_i(j, "room_vr_cubemap_face_size", s.room_vr_cubemap_face_size);
    s.room_vr_texture_uv_dim = get_i(j, "room_vr_texture_uv_dim", s.room_vr_texture_uv_dim);
    ...
    s.room_vr_cubemap_face_size = std::clamp(s.room_vr_cubemap_face_size, 32, 512);
    s.room_vr_texture_uv_dim = RoomSampleFrameProtocol::SnapUvTextureDim(
        std::clamp(s.room_vr_texture_uv_dim, 64, 4096));
