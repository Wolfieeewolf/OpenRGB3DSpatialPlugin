...
Lightning::Lightning(QWidget* parent) : SpatialEffect3D(parent)
{
    ...
    flash_rate_slider       = nullptr;
    flash_duration_slider   = nullptr;
    strike_rate = 5;
    branches = 6;
    cache_time = -1e9f;
...
void Lightning::SetupCustomUI(QWidget* parent)
{
    ...
    mode_combo->setItemData(0, QStringLiteral("Grounded energy sphere with branching arcs."), Qt::ToolTipRole);
    mode_combo->setItemData(1, QStringLiteral("Brief ceiling-wide flash."), Qt::ToolTipRole);
    mode_combo->setItemData(2, QStringLiteral("Single bolt from cloud band to floor."), Qt::ToolTipRole);
    mode_combo->setItemData(3, QStringLiteral("Forked discharge from cloud height."), Qt::ToolTipRole);
    mode_combo->setItemData(4, QStringLiteral("Multiple strikes and ambient flicker."), Qt::ToolTipRole);
    ...
    EffectSliderRow* arches_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Arches/sec (Plasma):"),
        1,
        20,
        (int)strike_rate,
        QStringLiteral("Number of new lightning arcs per second"));
    arches_row->setObjectName(QStringLiteral("archesPerSecRow"));
    strike_rate_slider = arches_row->slider();
    arches_row->bindValueChanged(
        this,
        [this](int v) {
            strike_rate = (unsigned int)v;
            OnLightningParameterChanged();
        },
        [](int v) { return QString::number(v); },
        nullptr);

    EffectSliderRow* max_arcs_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Max arcs (Plasma):"),
        2,
        12,
        (int)branches,
        QStringLiteral("Maximum simultaneous arcs (lower = better FPS)"));
    max_arcs_row->setObjectName(QStringLiteral("maxArcsRow"));
    branch_slider = max_arcs_row->slider();
    max_arcs_row->bindValueChanged(
        this,
        [this](int v) {
            branches = (unsigned int)v;
            OnLightningParameterChanged();
        },
        [](int v) { return QString::number(v); },
        nullptr);

    EffectSliderRow* sky_rate_row = EffectUiRows::AppendSliderRow(
        layout, QStringLiteral("Sky event rate:"), 5, 50, (int)(flash_rate * 100.0f));
    sky_rate_row->setObjectName(QStringLiteral("skyEventRateRow"));
    flash_rate_slider = sky_rate_row->slider();
    sky_rate_row->bindValueChanged(
        this,
        [this](int v) { flash_rate = v / 100.0f; },
        [this](int) { return QString::number(flash_rate, 'f', 2); },
        on_changed);

    EffectSliderRow* sky_duration_row = EffectUiRows::AppendSliderRow(
        layout, QStringLiteral("Sky event duration:"), 2, 25, (int)(flash_duration * 100.0f));
    sky_duration_row->setObjectName(QStringLiteral("skyEventDurationRow"));
    flash_duration_slider = sky_duration_row->slider();
    sky_duration_row->bindValueChanged(
        this,
        [this](int v) { flash_duration = v / 100.0f; },
        [this](int) { return QString::number(flash_duration * 1000.0f, 'f', 0) + QStringLiteral(" ms"); },
        on_changed);
...
void Lightning::OnLightningParameterChanged()
{
    if(strike_rate_slider)
        strike_rate = (unsigned int)strike_rate_slider->value();
    if(branch_slider)
        branches = (unsigned int)branch_slider->value();
    cache_time = -1e9f;
    emit ParametersChanged();
}
...
void Lightning::LoadSettings(const nlohmann::json& settings)
{
    ...
    if(strike_rate_slider)
        strike_rate_slider->setValue((int)strike_rate);
    if(branch_slider)
        branch_slider->setValue((int)branches);
    if(flash_rate_slider)
        flash_rate_slider->setValue((int)(flash_rate * 100.0f));
    if(flash_duration_slider)
        flash_duration_slider->setValue((int)(flash_duration * 100.0f));
    if(QWidget* panel = CustomSettingsPanelWidget())
    {
        EffectUiSync::setComboIndex(panel, "styleRow", mode);
    }
    cache_time = -1e9f;
}
