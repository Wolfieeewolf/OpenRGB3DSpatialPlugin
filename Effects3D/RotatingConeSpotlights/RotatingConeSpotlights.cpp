...
void RotatingConeSpotlights::SetupCustomUI(QWidget* parent)
{
    ...
    EffectSliderRow* cone_scale_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Cone scale:"),
        5,
        500,
        (int)std::lround(cone_scale * 1000.0f),
        QStringLiteral("1/(π²) is default; lower = tighter cones, higher = wider wash."));
    ...
    EffectSliderRow* hue_shift_row = EffectUiRows::AppendSliderRow(
        ...
        QStringLiteral("Added to hue from position along room X (reference pattern uses x as hue)."));
    ...
    EffectSliderRow* motion_row = EffectUiRows::AppendSliderRow(
        ...
        QStringLiteral("How fast the rotation axis and angle evolve."));
    motion_row->setObjectName(QStringLiteral("motionRow"));
    motion_slider = motion_row->slider();
    motion_row->bindValueChanged(
        this,
