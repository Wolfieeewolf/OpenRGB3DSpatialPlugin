...
void Starfield::SetupCustomUI(QWidget* parent)
{
    ...
    mode_combo->setToolTip(QStringLiteral(
        "Motion style for star points. Size and drift still apply in both modes."));
    mode_combo->setItemData(0,
                            QStringLiteral("Classic drift along depth with wrap—good volumetric read."),
                            Qt::ToolTipRole);
    mode_combo->setItemData(1,
                            QStringLiteral("Stars brighten and dim in place; less directional motion."),
                            Qt::ToolTipRole);
    ...
    EffectSliderRow* star_count_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Star count:"),
        40,
        120,
        num_stars,
        QStringLiteral("Number of star points simulated in the field."));
    star_count_row->setObjectName(QStringLiteral("starCountRow"));
    star_count_row->bindValueChanged(
        this, [this](int v) { num_stars = v; }, [](int v) { return QString::number(v); }, on_changed);

    EffectSliderRow* star_size_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Star size:"),
        2,
        100,
        (int)(star_size * 100.0f),
        QStringLiteral("Apparent size of each star streak or point."));
    star_size_row->setObjectName(QStringLiteral("starSizeRow"));
    star_size_row->bindValueChanged(
        this,
        [this](int v) { star_size = v / 100.0f; },
        [](int v) { return QString::number(v) + QStringLiteral("%"); },
        on_changed);

    EffectSliderRow* drift_row = EffectUiRows::AppendSliderRow(
        layout, QStringLiteral("Drift:"), 0, 100, (int)(drift_amount * 100.0f));
    drift_row->setObjectName(QStringLiteral("driftRow"));
    drift_row->bindValueChanged(
        this,
        [this](int v) { drift_amount = v / 100.0f; },
        [](int v) { return QString::number(v) + QStringLiteral("%"); },
        on_changed);

    EffectSliderRow* twinkle_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Twinkle:"),
        0,
        100,
        (int)(twinkle_speed * 100.0f),
        QStringLiteral(
            "Brightness modulation; strongest in Twinkle mode, "
            "subtle shimmer in Starfield when raised."));
    twinkle_row->setObjectName(QStringLiteral("twinkleRow"));
    twinkle_row->bindValueChanged(
        this,
        [this](int v) { twinkle_speed = v / 100.0f; },
        [](int v) { return QString::number(v) + QStringLiteral("%"); },
        on_changed);
