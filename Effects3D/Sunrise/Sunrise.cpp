...
{
    QWidget* w = EffectUiRows::NewEffectPanel("SunriseEffectSettings");
    QVBoxLayout* layout = EffectUiRows::PanelLayout(w);
    const auto on_changed = [this]() { emit ParametersChanged(); };

    EffectLabeledComboRow* time_mode_row = EffectUiRows::AppendComboRow(layout, QStringLiteral("Time mode:"));
    time_mode_row->setObjectName(QStringLiteral("timeModeRow"));
    QComboBox* mode_combo = time_mode_row->combo();
    ...
    mode_combo->setToolTip(QStringLiteral(
        "What drives the sun/sky cycle. Simulated uses Day length; Real-time uses your PC clock."));
    mode_combo->setItemData(0, QStringLiteral("Effect speed slider drives the day phase."), Qt::ToolTipRole);
    mode_combo->setItemData(1, QStringLiteral("Phase follows local time of day."), Qt::ToolTipRole);
    mode_combo->setItemData(2, QStringLiteral("Compresses a full day into Day length minutes."), Qt::ToolTipRole);
    ...
    EffectLabeledComboRow* color_preset_row = EffectUiRows::AppendComboRow(layout, QStringLiteral("Color preset:"));
    color_preset_row->setObjectName(QStringLiteral("colorPresetRow"));
    QComboBox* preset_combo = color_preset_row->combo();
    for(int p = 0; p < PRESET_COUNT; p++)
        preset_combo->addItem(PresetName(p));
    preset_combo->setCurrentIndex(std::max(0, std::min(color_preset, PRESET_COUNT - 1)));
    preset_combo->setToolTip(QStringLiteral(
        "Loads sky/horizon/ground key colors. Custom leaves your color pickers as-is."));
    preset_combo->setItemData(0, QStringLiteral("Warm dawn palette."), Qt::ToolTipRole);
    preset_combo->setItemData(1, QStringLiteral("Evening oranges and deep blues."), Qt::ToolTipRole);
    preset_combo->setItemData(2, QStringLiteral("Bright midday look."), Qt::ToolTipRole);
    preset_combo->setItemData(3, QStringLiteral("Dim night tones."), Qt::ToolTipRole);
    preset_combo->setItemData(4, QStringLiteral("No automatic color load—edit colors manually."), Qt::ToolTipRole);
    connect(preset_combo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this, on_changed](int idx) {
        color_preset = std::max(0, std::min(idx, PRESET_COUNT - 1));
        ApplyPreset(color_preset);
        on_changed();
    });

    EffectSliderRow* day_length_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Day length (min):"),
        1,
        120,
        (int)day_length_minutes,
        QStringLiteral("Length of one full day/night cycle in Simulated time mode (minutes)."));
    day_length_row->setObjectName(QStringLiteral("dayLengthRow"));
    day_length_row->bindValueChanged(
        this, [this](int v) { day_length_minutes = (float)v; }, [](int v) { return QString::number(v); },
        on_changed);

    auto add_weather_check = [&](const char* name, const QString& text, const QString& tip, bool& field) {
        auto* box = new QCheckBox(text, w);
        box->setObjectName(name);
        box->setToolTip(tip);
        box->setChecked(field);
        layout->addWidget(box);
        connect(box, &QCheckBox::toggled, this, [this, on_changed, &field](bool on) {
            field = on;
            on_changed();
        });
    };
    add_weather_check("rainCheck", QStringLiteral("Rain"),
                      QStringLiteral("Adds blue-violet streaks in the upper volume."), weather_rain);
    add_weather_check("fogCheck", QStringLiteral("Fog"),
                      QStringLiteral("Softens horizon contrast and lifts low tones."), weather_fog);
    add_weather_check("cloudyCheck", QStringLiteral("Cloudy"),
                      QStringLiteral("Broadens cloud band and mutes the sun disk."), weather_cloudy);
    add_weather_check("lightningCheck", QStringLiteral("Lightning"),
                      QStringLiteral("Random brief flashes during stormy parts of the cycle."),
                      weather_lightning);
...
void Sunrise::LoadSettings(const nlohmann::json& settings)
{
    ...
    {
        if(QWidget* fx = EffectUiSync::effectPanel(panel, "SunriseEffectSettings"))
        {
            EffectUiSync::setComboIndex(fx, "timeModeRow", time_mode);
            EffectUiSync::setComboIndex(fx, "colorPresetRow", color_preset);
            EffectUiSync::setSliderValue(fx, "dayLengthRow", (int)day_length_minutes,
                                          [](int v) { return QString::number(v); });
            EffectUiSync::setCheckBox(fx, "rainCheck", weather_rain);
            EffectUiSync::setCheckBox(fx, "fogCheck", weather_fog);
            EffectUiSync::setCheckBox(fx, "cloudyCheck", weather_cloudy);
            EffectUiSync::setCheckBox(fx, "lightningCheck", weather_lightning);
        }
    }
}
