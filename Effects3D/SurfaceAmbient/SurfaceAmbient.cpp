...
float SurfaceAmbient::EvalPlasmaStyle(int style, float u, float v, float dist_norm, float time, float freq, float speed)
{
...
void SurfaceAmbient::SetupCustomUI(QWidget* parent)
{
    ...
    style_combo->setItemData(0, QStringLiteral("Chaotic warm flicker—strong vertical motion."), Qt::ToolTipRole);
    style_combo->setItemData(1, QStringLiteral("Cool ripples and softer peaks."), Qt::ToolTipRole);
    style_combo->setItemData(2, QStringLiteral("Blobby organic motion."), Qt::ToolTipRole);
    style_combo->setItemData(3, QStringLiteral("Sharper hot peaks with radial emphasis."), Qt::ToolTipRole);
    style_combo->setItemData(4, QStringLiteral("Softer, wider fire—less harsh detail."), Qt::ToolTipRole);
    style_combo->setItemData(5, QStringLiteral("Deeper, slower waves."), Qt::ToolTipRole);
    style_combo->setItemData(6, QStringLiteral("Wispy high-frequency haze."), Qt::ToolTipRole);
    ...
    EffectSliderRow* height_row = EffectUiRows::AppendSliderRow(
        ...
        QStringLiteral(
            "How far the effect extends from each enabled shell into the room (surface mask in common controls)."));
