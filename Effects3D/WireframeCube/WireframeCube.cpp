...
EffectInfo3D WireframeCube::GetEffectInfo() const
{
    EffectInfo3D info{};
    info.info_version = 2;
...
{
    QWidget* w = EffectUiRows::NewEffectPanel("WireframeCubeEffectSettings");
    QVBoxLayout* layout = EffectUiRows::PanelLayout(w);
    const auto on_changed = [this]() { emit ParametersChanged(); };
    const auto pct_format = [](int v) { return QString::number(v) + QStringLiteral("%"); };

    EffectSliderRow* edge_glow_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Edge glow:"),
        2,
        100,
        (int)(thickness * 100.0f),
        QStringLiteral("How wide the lit region around each edge (halo thickness)."));
    edge_glow_row->setObjectName(QStringLiteral("edgeGlowRow"));
    edge_glow_row->bindValueChanged(
        this, [this](int v) { thickness = v / 100.0f; }, pct_format, on_changed);

    EffectSliderRow* line_brightness_row = EffectUiRows::AppendSliderRow(
        layout,
        QStringLiteral("Line brightness:"),
        0,
        100,
        (int)(line_brightness * 100.0f),
        QStringLiteral("Peak brightness along the wireframe lines."));
    line_brightness_row->setObjectName(QStringLiteral("lineBrightnessRow"));
    line_brightness_row->bindValueChanged(
        this, [this](int v) { line_brightness = v / 100.0f; }, pct_format, on_changed);
...
RGBColor WireframeCube::CalculateColorGrid(float x, float y, float z, float time, const GridContext3D& grid)
{
    ...
    EffectGridAxisHalfExtents wf_e = MakeEffectGridAxisHalfExtents(grid, GetNormalizedScale());
    const float wf_h_scale = std::max({wf_e.hw, wf_e.hh, wf_e.hd});
    const float thickness_world = std::max(thickness, 0.02f) * wf_h_scale * 0.25f;
    float sigma = std::max(thickness_world, 1e-3f);
    ...
    float hue = std::fmod(path01 * 360.0f * (0.6f + 0.4f * detail) + time * speed_phase  + EffectStratumBlend::CombinedPhase01(bb, stratum_mot01) * 360.0f,
                          360.0f);
    if(hue < 0.0f)
        hue += 360.0f;
    ...
    {
        const float dir01 = std::fmod(GetEffectStripColormapDirectionDeg() * (1.f / 360.f), 1.f);
...
room_wf_cache_seq = 0;
    ...
    {
        if(QWidget* fx = EffectUiSync::effectPanel(panel, "WireframeCubeEffectSettings"))
        {
            const auto pct = [](int v) { return QString::number(v) + QStringLiteral("%"); };
            EffectUiSync::setSliderValue(fx, "edgeGlowRow", (int)(thickness * 100.0f), pct);
            EffectUiSync::setSliderValue(fx, "lineBrightnessRow", (int)(line_brightness * 100.0f), pct);
        }
    }
}
