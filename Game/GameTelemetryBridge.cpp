                                                  ...
                                                  std::vector<unsigned char> rgba)
{
    ...
    // Accept frames while the mod catches up after a settings toggle (sky/UV/face).
    // Only reject when the cubemap dimensions disagree — config_id churn used to black out LEDs.
    if(hdr.size_x != cfg.size_x || hdr.size_y != cfg.size_y || hdr.size_z != cfg.size_z)
    {
        return;
    }
