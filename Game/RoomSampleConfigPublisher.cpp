...
/** Cubemap texels covering each mapped LED direction (nearest). */
                                        ...
                                        std::vector<std::uint32_t>& out)
{
    ...
    const std::size_t led_count = led_xyz.size() / 3u;
    // Even stride when denser than the unique-texel budget — first-N left lower wallpaper rows black.
    const std::size_t step = std::max<std::size_t>(
        1u, (led_count + RoomSampleFrameProtocol::kMaxImportantCells - 1u)
                / RoomSampleFrameProtocol::kMaxImportantCells);

    std::unordered_set<std::uint32_t> unique;
    unique.reserve(std::min(led_count / step + 8u, (std::size_t)RoomSampleFrameProtocol::kMaxImportantCells));
    ...
    for(std::size_t li = 0; li < led_count; li += step)
    {
        const std::size_t i = li * 3u;
...
void SetFrameLedRoomPositions(const float* xyz_triplets, std::size_t triplet_count)
{
    ...
    const std::size_t max_triplets = RoomSampleFrameProtocol::kMaxLedPositions;
                     ...
                     float room_to_world_scale)
{
    ...
    if(config_changed)
    {
        LOG_INFO("[3DSpatial] room config published (cubemap %dx%d x6, UV %u, config id %u, scale %.4f, "
                 "room spans %.1f/%.1f/%.1f, layout LEDs %zu, mapped LED texels %zu)",
                 nx,
                 ny,
                 uv_dim,
                 hdr.config_id,
        ...
        if(led_xyz.size() / 3u > g_last_important_cells.size())
        {
            LOG_INFO("[3DSpatial] room config: layout LEDs exceed unique mapped texels — "
                     "dense wallpaper rows share neighbours (cap %u)",
                     RoomSampleFrameProtocol::kMaxImportantCells);
        }
    }
