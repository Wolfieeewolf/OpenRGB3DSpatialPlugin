...
/** Unused — kept so flag bit indices stay stable (dense/LZ4 frames removed). */
...
/**
 * Frame payload is sparse LED texels: uint32 count, then count × (uint32 flat, u8 r,g,b,a).
 * Avoids allocating face²×6×4 (breaks ~face 390+ under heap/SHM pressure).
 */
constexpr std::uint32_t kFlagSparseLedTexels = 1u << 7;
constexpr std::size_t kDefaultTargetCells = 800u * 600u;
/** Max dense cubemap cells after sparse expand (512²×6). */
constexpr std::size_t kMaxCells = 512u * 512u * 6u;
/**
 * Unique LED-mapped cubemap texels published to the mod.
 * Wallpaper Engine display matrices often exceed 16k LEDs; keep headroom for ~256×144 + extras.
 */
constexpr std::uint32_t kMaxImportantCells = 65536u;
/** Max LED room positions retained when building the important set (may exceed unique texels). */
constexpr std::uint32_t kMaxLedPositions = 131072u;
...
inline bool TryComputeRgbaBytes(int sx, int sy, int sz, std::size_t& out_bytes)
{
    ...
    return out_bytes / 4u <= kMaxCells;
}

/** Sparse LED payload: uint32 count + count × (uint32 flat, u8 r,g,b,a). */
inline std::size_t SparseLedPayloadBytes(std::uint32_t count)
{
    return 4u + (std::size_t)count * 8u;
}
