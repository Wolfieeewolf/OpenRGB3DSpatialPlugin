                         ...
                         std::vector<unsigned char>& rgba_out)
{
    ...
    // Sparse LED payload only — dense/LZ4 frames are not accepted (current mod contract).
    if((hdr.flags & RoomSampleFrameProtocol::kFlagSparseLedTexels) == 0)
    {
        return false;
    }
    if(hdr.rgba_stored_size < 4u)
    {
        return false;
    }
    rgba_out.assign(raw_bytes, 0);
    const std::uint32_t count = (std::uint32_t)payload[0]
                                | ((std::uint32_t)payload[1] << 8)
                                | ((std::uint32_t)payload[2] << 16)
                                | ((std::uint32_t)payload[3] << 24);
    const std::size_t need = RoomSampleFrameProtocol::SparseLedPayloadBytes(count);
    if(count > RoomSampleFrameProtocol::kMaxImportantCells || need != hdr.rgba_stored_size)
    {
        return false;
    }
    ...
    {
        const std::uint32_t flat = (std::uint32_t)p[0]
                                  | ((std::uint32_t)p[1] << 8)
                                  | ((std::uint32_t)p[2] << 16)
                                  | ((std::uint32_t)p[3] << 24);
        p += 4;
        const std::size_t o = (std::size_t)flat * 4u;
        if(o + 4u <= raw_bytes)
        {
            rgba_out[o] = p[0];
            rgba_out[o + 1] = p[1];
