...
bool SpatialStripFieldEngine::initGl()
{
    ...
    if(!SpatialOffscreenGlPool::warmUp(&last_error_))
    {
        return false;
    }
    gl_ok_ = true;
...
bool SpatialStripFieldEngine::compileProgram(const QString& body)
{
    ...
    if(!ctx || !ctx->isValid())
    {
        last_error_ = QStringLiteral("OpenGL context unavailable.");
        return false;
    }
...
bool SpatialStripFieldEngine::renderStrip()
{
    ...
    QOpenGLFunctions* glf = SpatialOffscreenGlPool::sharedContext()->functions();
...
{
    QOpenGLFunctions* gl = SpatialOffscreenGlPool::sharedContext()->functions();
