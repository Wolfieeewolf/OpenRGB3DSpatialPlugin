                                  ...
                                  bool& out_valid)
{
    ...
    const int iu0 = std::clamp((int)std::lround(gu), 0, rs.size_x - 1);
    const int iv0 = std::clamp((int)std::lround(gv), 0, rs.size_y - 1);

    auto try_texel = [&](int iu, int iv) -> RGBColor {
    ...
    RGBColor c = try_texel(iu0, iv0);
    if(out_valid)
    {
        return c;
    }

    // Stride-sampled important sets leave empty gaps on dense wallpaper matrices —
    // borrow the nearest filled neighbour before painting black.
    for(int radius = 1; radius <= 3; ++radius)
    {
        for(int dv = -radius; dv <= radius; ++dv)
        {
            for(int du = -radius; du <= radius; ++du)
            {
                ...
                if(std::max(std::abs(du), std::abs(dv)) != radius)
                {
                    continue;
                }
                const int iu = iu0 + du;
                const int iv = iv0 + dv;
                if(iu < 0 || iv < 0 || iu >= rs.size_x || iv >= rs.size_y)
                {
                    continue;
                }
                c = try_texel(iu, iv);
