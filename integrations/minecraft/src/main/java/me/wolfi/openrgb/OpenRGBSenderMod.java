...
public class OpenRGBSenderMod implements ClientModInitializer
{
    ...
    /** Compact LED-texel RGBA (important.length × 4) — not a dense cubemap. */
    private byte[] roomSampleRgbaBuffer = new byte[0];
    /** Soft ceiling for UV/face-cache drain after a room fill. */
    private static final long CLIENT_TICK_WORK_BUDGET_NS = 6_000_000L;
    /** Only skip a tick after a clear whole-frame hitch. */
    private static final long ROOM_SAMPLE_SOFT_HITCH_NS = 8_000_000L;
    ...
    /** When LED-first config changes, soft-start without wiping live colours. */
    private int roomSampleClearedForConfigId = -1;
    /** Last whole-frame fill duration — mild adaptive interval only. */
    private long lastRoomFillNs = 0L;
    ...
    public void onInitializeClient()
    {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(OpenRGBSenderMod::onClientTick);
        // Sparse mmap publish + UDP notify (LED telemetry only — no world mutation).
        ...
        final Identifier precacheId =
                Identifier.fromNamespaceAndPath("openrgb-minecraft-sender", "block_model_precache");
    ...
    /**
     * End of each client tick via {@link net.fabricmc.fabric.api.client.event.lifecycle.v1.ClientTickEvents}
     * (main thread only). Samples the world for OpenRGB; does not alter gameplay.
     */
    public static void onClientTick(Minecraft client)
    {
        ...
        // Whole LED frames first — precache must not starve or partial-fill strips.
    ...
    private void tickRoomSamples(Minecraft client)
    {
        ...
        try
        {
            ...
            final int roomInterval = effectiveRoomSampleInterval(cfg);
    ...
    /**
     * Sky only on empty rays that aim upward. Eye-level / horizontal misses (forest gaps,
     * horizon through foliage) stay dark so desk/monitor-back LEDs don't turn sky-blue.
     */
    private static final float SKY_FILL_MIN_LOCAL_UP = 0.35f;
                                            ...
                                            int[] cell)
    {
        ...
        if(localUp < SKY_FILL_MIN_LOCAL_UP)
        {
            return;
        }
        ...
        // Fade sky in with elevation so the horizon band stays soft.
        final float elev = Math.min(1.0f, (localUp - SKY_FILL_MIN_LOCAL_UP) / (1.0f - SKY_FILL_MIN_LOCAL_UP));
        final int skyA = ColorMath.clamp255(Math.round((28 + skyLight * 3) * (0.45f + 0.55f * night) * elev));
        cell[3] = skyA;
    }

    /** Prefer every-tick whole frames; only skip a tick after a clear hitch. */
    private int effectiveRoomSampleInterval(OpenRGBSenderConfig cfg)
    {
        int interval = Math.max(1, cfg.roomSampleSendInterval);
        if(lastRoomFillNs >= ROOM_SAMPLE_SOFT_HITCH_NS)
        {
                                     ...
                                     RoomSampleConfigReader.Config cfg) throws Exception
    {
        ...
        final int ledBytes = n * 4;
        if(roomSampleRgbaBuffer.length != ledBytes)
        {
            roomSampleRgbaBuffer = new byte[ledBytes];
        ...
        try
        {
            ...
            for(int i = 0; i < n; i++)
            {
                ...
                writeCubemapTexel(world, player, cfg, eye, atmosphere, localDir, cell, rgba, i, flat, sx, sy, sz);
            }
            lastRoomFillNs = System.nanoTime() - fillStart;
        }
        finally
        {
        ...
        roomSampleFrameShmWriter.publishSparseLedTexels(
                frameId, now, cfg.configId, sx, sy, sz, important, rgba);
    }
                                          ...
                                          int sz)
    {
        ...
        maybeFillOutdoorSky(cfg, world, eye, target, atmosphere, localDir[1], cell);
        final int rgbaIndex = compactIndex * 4;
