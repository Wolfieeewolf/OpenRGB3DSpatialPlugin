...
void LEDViewport3D::DrawLEDs(ControllerTransform* ctrl)
{
    ...
    led_draw_positions.resize(led_count * 3);
    led_draw_colors.resize(led_count * 3);
    float* pos_out = led_draw_positions.data();
    float* col_out = led_draw_colors.data();
    ...
    if(ctrl->virtual_controller)
    {
        ...
        for(size_t i = 0; i < led_count; i++)
        {
            ...
            *pos_out++ = pos.local_position.x;
            *pos_out++ = pos.local_position.y;
            *pos_out++ = pos.local_position.z;
            *col_out++ = (float)RGBGetRValue(color) / 255.0f;
            *col_out++ = (float)RGBGetGValue(color) / 255.0f;
            *col_out++ = (float)RGBGetBValue(color) / 255.0f;
        }
    }
    else
    {
    ...
    glDrawArrays(GL_POINTS, 0, (GLsizei)led_count);
    glDisableClientState(GL_COLOR_ARRAY);
    glDisableClientState(GL_VERTEX_ARRAY);
    glPointSize(1.0f);
    glColor3f(1.0f, 1.0f, 1.0f);
