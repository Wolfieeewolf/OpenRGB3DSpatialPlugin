...
void OpenRGB3DSpatialTab::createCustomControllerClicked()
{
    ...
    std::unique_ptr<VirtualController3D> virtual_ctrl = std::make_unique<VirtualController3D>(
        ...
        dialog.GetColumnWidthsMm(),
        dialog.GetRowHeightsMm(),
        dialog.GetLayerDepthsMm(),
        dialog.GetLayerNames(),
        dialog.GetLedsPerCluster()
    );
    ...
    SaveCustomController(static_cast<unsigned int>(new_index));
...
void OpenRGB3DSpatialTab::editCustomControllerClicked()
{
    ...
    dialog.LoadExistingController(virtual_ctrl->GetName(),
                                  ...
                                  virtual_ctrl->GetColumnWidthsMm(),
                                  virtual_ctrl->GetRowHeightsMm(),
                                  virtual_ctrl->GetLayerDepthsMm(),
                                  virtual_ctrl->GetLayerNames(),
                                  virtual_ctrl->GetLedsPerCluster());
    ...
    if(dialog.exec() == QDialog::Accepted)
    {
        ...
        SaveCustomController(static_cast<unsigned int>(list_row));
        UpdateAvailableControllersList();
...
int OpenRGB3DSpatialTab::IndexOfVirtualController(const VirtualController3D* controller) const
{
    if(!controller)
    {
        return -1;
    ...
    {
        if(virtual_controllers[i].get() == controller)
        {
...
bool OpenRGB3DSpatialTab::SaveCustomController(unsigned int index)
{
    if(!resource_manager || index >= virtual_controllers.size() || !virtual_controllers[index])
    {
        return false;
    }
    ...
    const std::string json_filename = SafeControllerJsonFilename(virtual_controllers[index]->GetName());
    const filesystem::path filepath = controllers_dir / json_filename;

    if(index < virtual_controller_json_files.size()
       && !virtual_controller_json_files[index].empty()
       && virtual_controller_json_files[index] != json_filename)
    {
        const filesystem::path old_path = controllers_dir / virtual_controller_json_files[index];
        std::error_code remove_ec;
    ...
    try
    {
        nlohmann::json ctrl_json = virtual_controllers[index]->ToJson();
...
bool OpenRGB3DSpatialTab::CustomControllerHasUndetectedDevices(const VirtualController3D* virtual_ctrl) const
{
    if(!virtual_ctrl)
    {
        return true;
    ...
    {
        if(!mapping.controller_name.empty() && !mapping.controller)
        {
            return true;
        }
    }
...
void OpenRGB3DSpatialTab::MarkControllerLedsUsed(RGBControllerInterface* controller,
                                                 std::vector<bool>& used_leds) const
{
    if(!controller || used_leds.empty())
    {
        return;
    }
    ...
    // LEDs claimed by custom controller definitions (even when not yet placed in the scene)
    // are unavailable for direct device/zone/LED add — only the custom entry remains.
    for(unsigned int i = 0; i < virtual_controllers.size(); i++)
    {
        const VirtualController3D* virtual_ctrl = virtual_controllers[i].get();
        if(!virtual_ctrl)
        {
            continue;
        }
...
bool OpenRGB3DSpatialTab::IsItemInScene(RGBControllerInterface* controller, int granularity, int item_idx) const
{
    ...
    MarkControllerLedsUsed(controller, used_leds);
    ...
    else if(granularity == 1)
    {
        ...
        for(unsigned int led_idx = 0; led_idx < selected_zone.leds_count; led_idx++)
        {
            ...
            if(!IsAssignableControllerLed(controller, global_led_idx))
            {
                continue;
            }
        ...
        // Zones with only blank filler slots (or fully assigned) are not available to add.
        return true;
    }
    else if(granularity == 2)
    {
        ...
        if(!IsAssignableControllerLed(controller, led_idx))
        {
            return true;
        }
...
int OpenRGB3DSpatialTab::GetUnassignedLEDCount(RGBControllerInterface* controller) const
{
    ...
    for(unsigned int i = 0; i < used_leds.size(); i++)
    {
        if(IsAssignableControllerLed(controller, i) && !used_leds[i])
        {
            free_count++;
        }
    }
